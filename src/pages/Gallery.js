...
export default function Gallery() {
    ...
    const filtered = useMemo(() => (tag === 'all' ? galleryItems : galleryItems.filter((g) => g.tag === tag)), [tag]);
    const visible = filtered.slice(0, visibleCount);
    const canLoadMore = visibleCount < filtered.length;
    const lightboxList = filtered;
    const lightboxOpen = lightboxIndex !== null;
    const currentSrc = lightboxIndex !== null && lightboxList[lightboxIndex]
    ...
    return (_jsxs(Box, { component: "main", children: [_jsx(Box, { sx: {
                    ...
                    borderColor: 'divider',
                }, children: _jsxs(Container, { maxWidth: "lg", children: [_jsx(Typography, { variant: "h1", sx: { fontSize: { xs: '1.85rem', md: '2.35rem' }, fontWeight: 800, mb: 1 }, children: "Our life through the lens" }), _jsx(Typography, { color: "text.secondary", sx: { maxWidth: 640, lineHeight: 1.75, mb: 1 }, children: "Moments from projects, workshops, and the people who make MITRA happen. Tap a photo \u2014 browse full screen with arrows." }), _jsxs(Link, { href: "https://instagram.com", target: "_blank", rel: "noopener noreferrer", sx: { display: 'inline-flex', alignItems: 'center', gap: 0.75, fontWeight: 600 }, underline: "hover", children: [_jsx(InstagramIcon, { fontSize: "small" }), "More on Instagram"] })] }) }), _jsxs(Container, { maxWidth: "lg", sx: { py: { xs: 3, md: 4 } }, children: [_jsx(Box, { sx: { mb: 2, overflowX: 'auto' }, children: _jsx(ToggleButtonGroup, { value: tag, exclusive: true, onChange: handleTag, size: "small", "aria-label": "Gallery filter", sx: {
                                flexWrap: 'wrap',
                                gap: 0.5,
                                '& .MuiToggleButton-root': { textTransform: 'none', borderRadius: '999px !important', px: 1.5 },
                            }, children: tagOptions.map((opt) => (_jsx(ToggleButton, { value: opt.value, children: opt.label }, opt.value))) }) }), _jsx(Grid, { container: true, spacing: 1.5, children: visible.map((item) => (_jsx(Grid, { size: { xs: 6, md: item.featured ? 8 : 4 }, children: _jsx(Box, { onClick: () => openAt(item), onKeyDown: (e) => {
                                    if (e.key === 'Enter' || e.key === ' ') {
                                        e.preventDefault();
                                        openAt(item);
                                    }
                                }, role: "button", tabIndex: 0, sx: {
                                ...
                                }, children: _jsx(Box, { component: "img", src: getImagePath(item.path), alt: item.alt, loading: "lazy", sx: { width: '100%', height: '100%', objectFit: 'cover', display: 'block' } }) }) }, item.id))) }), filtered.length === 0 && (_jsx(Typography, { color: "text.secondary", sx: { py: 4 }, children: "Nothing in this filter." })), canLoadMore && (_jsx(Box, { sx: { textAlign: 'center', mt: 3 }, children: _jsx(Button, { variant: "outlined", onClick: () => setVisibleCount((n) => Math.min(n + LOAD, filtered.length)), children: "Load more" }) }))] }), _jsx(Dialog, { open: lightboxOpen, onClose: () => setLightboxIndex(null), maxWidth: false, PaperProps: {
                    sx: {
                ...
                }, children: _jsxs(Box, { sx: { position: 'relative', width: '100%', height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', p: 2 }, children: [_jsx(IconButton, { onClick: () => setLightboxIndex(null), sx: { position: 'absolute', top: 8, right: 8, color: 'common.white', zIndex: 2 }, "aria-label": "Close", children: _jsx(CloseIcon, {}) }), _jsx(IconButton, { onClick: goPrev, sx: { position: 'absolute', left: 8, top: '50%', transform: 'translateY(-50%)', color: 'common.white', zIndex: 2 }, "aria-label": "Previous", children: _jsx(ChevronLeftIcon, { sx: { fontSize: 40 } }) }), _jsx(IconButton, { onClick: goNext, sx: { position: 'absolute', right: 8, top: '50%', transform: 'translateY(-50%)', color: 'common.white', zIndex: 2 }, "aria-label": "Next", children: _jsx(ChevronRightIcon, { sx: { fontSize: 40 } }) }), lightboxOpen && (_jsx(Box, { component: "img", src: currentSrc, alt: "", sx: { maxWidth: '100%', maxHeight: '100%', objectFit: 'contain' } })), lightboxIndex !== null && lightboxList.length > 0 && (_jsxs(Typography, { sx: { position: 'absolute', bottom: 24, left: '50%', transform: 'translateX(-50%)', color: 'common.white', fontSize: '0.9rem' }, children: [lightboxIndex + 1, " / ", lightboxList.length] }))] }) })] }));
}
